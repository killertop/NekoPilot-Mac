...
> 当前仓库仍在持续开发。当前本地 Release 包适合开发和 QA；对外分发包必须通过 GitHub 上的 ad-hoc 签名发布流水线生成，详见 [docs/RELEASE.md](docs/RELEASE.md)。
...
当前产品与验收目标是 macOS。Linux 配置仍保留在 Tauri 基线中，可以作为次要平台生成软件包。Windows 源码仅作为兼容性保留，GitHub Release 不构建、不发布 Windows 安装包。
...
本地产物位于 `src-tauri/target/release/bundle/` 下。GitHub-only 发布路径使用 ad-hoc 签名，不包含 Apple 公证；首次打开和验证要求见 [docs/RELEASE.md](docs/RELEASE.md)。
