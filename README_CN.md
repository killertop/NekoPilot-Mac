...
旧的 Tauri/React/Rust 实现仍保留在 `src/` 和 `src-tauri/`，仅用于回滚与行为对照，不属于正式构建输入，也不会发布。
...
src/、src-tauri/                旧版回滚/行为对照代码
docs/                           开发与发布文档
```

更多信息见 [英文说明](README.md)、[开发指南](docs/DEVELOPMENT.md) 和 [发布指南](docs/RELEASE.md)。源码通过发布文档说明的 VPS 裸仓库同步到 [killertop/NekoPilot-Mac](https://github.com/killertop/NekoPilot-Mac)。许可证见 [LICENSE](LICENSE) 与 [NOTICE](NOTICE)。
