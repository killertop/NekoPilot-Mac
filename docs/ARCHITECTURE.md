...
NekoPilot communicates with sing-box through generated JSON configuration and the official 1.14 `StartedService` gRPC interface. Each sing-box process listens only on `127.0.0.1` at a newly allocated local port; Swift supplies a fresh 256-bit authorization secret in the owner-only (`0600`) runtime configuration for that process. There is no Dashboard, remote API setting, HTTP controller, Unix-socket bridge, custom Go source, or Clash service. Swift owns process lifecycle and performs live reload as a dual-core handoff: the candidate process gets an isolated API, proxy, health, and cache set, is validated end to end, then receives system-proxy ownership before the old process is stopped. A failed candidate is discarded while the old process and proxy ownership remain untouched.
